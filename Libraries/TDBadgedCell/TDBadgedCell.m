...
- (void) layoutSubviews
{
	...
	if(self.badgeString)
	{
        ...
        int xOrigin = 0;
        if (self.badgeLeftOffset > 0) {
            xOrigin = self.contentView.frame.origin.x + 13 + self.badgeLeftOffset;
        } else {
            xOrigin = self.contentView.frame.size.width - (badgeSize.width + 13 + self.badgeRightOffset);
        }
		CGRect badgeframe = CGRectMake(xOrigin,
